...
export default function ProfileCard({
  ...
  const currentPhoto =
    ...
    'https://ext.same-assets.com/3381471116/3994213222.jpeg'
          ...
          <div className='flex items-center justify-between'>
            <div>
              <h3 className='text-2xl font-bold flex items-center'>
                {user.name}, {user.age}
                <span
                  className={`ml-2 inline-block w-3 h-3 rounded-full ${facultyColor}`}
                ></span>
              </h3>
        ...
        <DialogContent className='max-w-md'>
          <DialogHeader>
            <DialogTitle className='flex items-center gap-2'>
              ...
              <span
                className={`inline-block w-3 h-3 rounded-full ${facultyColor}`}
              ></span>
            </DialogTitle>
            <DialogDescription>
              <div className='flex items-center gap-2 mt-1'>
                <span className={`font-medium ${facultyTextColor}`}>
                  {user.faculty}
                </span>
                <span>•</span>
                <span>
